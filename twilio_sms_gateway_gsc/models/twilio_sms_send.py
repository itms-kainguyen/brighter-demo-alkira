...
class TwilioSmsSend(models.Model):
    ...
    def _compute_twilio_response_data(self):
        for sms_id in self:
            ...
            sms_id.total_successfully_send_messages = len(
                sms_id.sms_log_history_ids.filtered(lambda x: x.status in SUCCESS_RESPONSE_STATUES))
            sms_id.total_error_messages = len(
                sms_id.sms_log_history_ids.filtered(lambda x: x.status not in SUCCESS_RESPONSE_STATUES))
    ...
    partner_ids = fields.Many2many("res.partner", "twilio_sms_send_partners_rel", "twilio_sms_send_id", "partner_id",
                                   "Contacts")
    ...
    total_successfully_send_messages = fields.Integer("Total Successfully Send Messages",
                                                      compute="_compute_twilio_response_data")
    ...
    state = fields.Selection(STATUS_SELECTION, string="Status", readonly=True, copy=False, default='draft',
                             required=True)
    sms_template_id = fields.Many2one("twilio.sms.template", "SMS Template", domain="[('model_id', '=', False)]",
                                      copy=False, )
