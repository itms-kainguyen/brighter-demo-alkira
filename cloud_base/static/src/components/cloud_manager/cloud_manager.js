...
export class CloudManager extends Component {
    ...
    */
    async _loadMassActions(props) {
        var massActions = await this.orm.call(
            componentModel,
            "action_return_mass_actions",
            [],
        );
    ...
    */
    async _onProceedMassAction(massActionID) {
        ...
        const actionResult = await this.orm.call(
            componentModel,
            "action_proceed_mass_action",
            [recordIds, massActionID],
        );
...
CloudManager.template = "cloud_base.CloudManager";
