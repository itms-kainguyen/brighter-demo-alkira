...
class sync_model(models.Model):
    ...
    @api.depends(
        "period_ids", "period_ids.field_id", "period_ids.period_value", "period_ids.period_type", 
...
distinguished.
For example, to achieve the structure 'Odoo > Contacts > Agrolait > Tasks > Task 1', you should define the rule (A) for\
Odoo partners and the rule (B) for Odoo tasks. Rule A should be a parent for rule B, as a linked field use 'Partner'""",
    )
...
not be synced even if they have quotations-subdirectories with attachments""",
    ...
    period_ids = fields.One2many("cloud.domain.period", "sync_model_id", string="Periods")
