        ...
        if calendar_active_id.partner_ids:
            ...
            try:
                ...
                if response.status_code == 204:
                    ...
                    return self.env['wk.wizard.message'].genrated_message("User is not found else meeting has been deleted", name='Message')
                else:
                    calendar_active_id.meeting_id = False
                    return self.env['wk.wizard.message'].genrated_message("There is a problem please confirm that meeting url is active or meeting is not expired", name='Message')
            except:
                return self.env['wk.wizard.message'].genrated_message("There is a problem in fields data or access token, please check and try again", name='Message')
    ...
    def delete_zoom_meeting(self):
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                return self.env['wk.wizard.message'].genrated_message("User is not found else meeting has been deleted already", name='Message')
            else:
                calendar_active_id.meeting_id = False
                return self.env['wk.wizard.message'].genrated_message("There is a problem please confirm that all details are proper and user belong to account", name='Message')
