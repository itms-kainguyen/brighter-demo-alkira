...
class WebsiteCalendar(http.Controller):
    ...
    def calendar_appointment_submit(self, appointment_type, country_id=False, **kwargs):
        ...
        categ_id = request.env.ref(
            'doctor_appointment_booking_advance_axis.calendar_event_type_data_online_appointment')
        ...
        calendar_id = request.env['calendar.event'].search([('id', '=', event.id)])
        calendar_id.update({
            'apppoint_event_id': "APPOINT" + str(event.id),
            'amount': partner.appointment_charge,
            'state': calendar_attendee.state
        })
        product = request.env['product.product'].sudo().search(
            [('name', 'ilike', appoint_group_id.product_template_id.name)])
        ...
        value = {
            ...
            'show_tokenize_input': PaymentPortal._compute_show_tokenize_input_mapping(
                acquirers, logged_in=logged_in, **kwargs
            ),
            'order_id': order.id,
            'access_token': str(uuid.uuid4()),
            'success_url': 'shop/payment',
            'error_url': 'error',
            'callback_method': 'callback_method',
            'order': order,
            'transaction_route': '/shop/payment/transaction/'+str(order.id),
...
class CustomerPortal(CustomerPortal):
    def _prepare_portal_layout_values(self):
        ...
        if is_admin:
            ...
            values['appointment_count'] = request.env['calendar.event'].sudo().search_count([('attendee_ids.partner_id','=',user.partner_id.id)])
    ...
    def my_appointments(self, page=1, date_begin=None, date_end=None, sortby=None, search=None, search_in='content', **kw):
        ...
        if is_admin:
            ...
            appointments = request.env['calendar.event'].sudo().search([('attendee_ids.partner_id','=',user.partner_id.id)])
        values.update({
            'appointments': appointments,
        })
